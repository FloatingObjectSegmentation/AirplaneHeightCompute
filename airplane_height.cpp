...
string dmr_directory = "E:\\workspaces\\LIDAR_WORKSPACE\\dmr";
string dmr_file_name = "pcd386_95";
string aug_directory = "C:\\Users\\km\\Desktop";
...
int main (int argc, char** argv)
{
	...
	std::vector<float> airplaneHeights;

	std::vector<int> pointIdxRadiusSearch;
	std::vector<float> pointRadiusSquaredDistance;

	for (int i = 0; i < augs.centerPointIndices.size(); i++)
	{

		pcl::PointXYZ pp(augs.points[augs.centerPointIndices[i]].x, augs.points[augs.centerPointIndices[i]].y, 0.0f); // projected point

		if (kdtree.radiusSearch(pp, sqrt(2 * pow(dmr_density, 2)) + 1.0f, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0)
		{
			...
			else 
			{
				...
				airplaneHeights.push_back(dmr.z_vals[min_r_idx]);
			}
		}
	}
	...
	ofstream outfile;
	outfile.open(aug_directory + "\\" + "airplaneheights" + aug_file_name, ios::out);
	stringstream ss;
	for (auto & i : airplaneHeights) {
		ss << i << " ";
	}
